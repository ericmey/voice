...
Monorepo for the voice stack: SIP trunking, realtime voice agents, and
...
├── Dockerfile.agent               builds voice-agent:latest
...
| Agents | Personas, prompts, voices, models | `agents/*`, `docker-compose.agents.yaml` |
...
| Supervisor | docker compose runs the agent containers | `docker-compose.agents.yaml`, `Dockerfile.agent`, `make deploy`, `make cycle` |
...
|------------|-------------|---------|
| `secrets/livekit-agents.env` | `config/secrets.env.example` | Provider keys, Musubi/OTLP endpoints, per-agent tokens |
...
make up                     # docker compose up -d (infra only)
make register-sip           # register trunk + dispatch rules from config
make deploy                 # build voice-agent:latest + bring up the four agents
...
| `make up` / `make down` | Bring the infra docker-compose stack up/down |
| `make deploy` | Build `voice-agent:latest` + bring up infra and the four agents |
| `make cycle` | Rebuild the image + recreate the agent containers (picks up code changes) |
...
your own LiveKit, SIP, provider API, Musubi, and OTLP configuration.
