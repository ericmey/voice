...
render_plist() {
  ...
  sed \
    ...
    -e "s|{{OPENCLAW_DEPLOYMENT_ENVIRONMENT}}|${OPENCLAW_DEPLOYMENT_ENVIRONMENT:-harem-world}|g" \
    -e "s|{{OPENCLAW_SERVICE_VERSION}}|${OPENCLAW_SERVICE_VERSION:-dev}|g" \
    -e "s|{{OPENCLAW_OTLP_ENDPOINT}}|${OPENCLAW_OTLP_ENDPOINT:-http://shiori.mey.house:4318/v1/traces}|g" \
